                ...
                if memName is not False:
                    ...
                    for word in question.split():
                        if word == ',' or word == " ":
                            ...
                            q_dict[word.lower().replace(" ", "")] = q_dict.get(word.lower(), 0) + 1
...
keySet = set([])
for dict in q_bank:
    keySet = keySet.union(set(dict.keys()))
...
def d_dict_to_csv(dic):
    # keys = dic[0].keys()
    with open('altqbank.csv', 'w', newline='',encoding='utf-8') as output_file:
...
# print(next(hansard_reader('qbank.csv')))
# print(next(next(hansard_reader('qbank.csv'))))
# print(next(next(next(hansard_reader('qbank.csv')))))